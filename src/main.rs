...
async fn main() {
    ...
    let initial_tls = tls::initialize_tls(&client, &svc_name, &namespace, &webhook_name).await;
    ...
    tokio::spawn(async move {
        tls::start_ha_tls_manager(client_clone, tls_config_handle, namespace, svc_name, webhook_name, initial_tls).await;
    });

    let app = Router::new().route("/mutate", post(webhook::mutate_handler));
    let addr = SocketAddr::from(([0, 0, 0, 0], 8443));

    info!("RustJack is ready and listening on {}", addr);
